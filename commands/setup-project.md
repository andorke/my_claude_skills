...
### Обновление документов проекта после OpenSpec changes
После завершения каждого `/opsx:apply` (когда все задачи отмечены) — проверь и обнови глобальные документы проекта, если change затронул их scope:
- **`CONTEXT.md`** — **всегда** добавь запись в журнал «что сделано» (новая строка сверху таблицы). Обнови «Текущая задача» и «Следующие шаги».
- **`ARCHITECTURE.md`** — обнови если change добавил новый компонент, интерфейс, слой или изменил архитектурное решение.
- **`DEVELOPMENT.md`** — обнови если change добавил новую зависимость, изменил структуру репозитория или паттерны кода.
- **`REQUIREMENTS.md`** — обнови если change реализовал FR/NFR (отметь как реализованное) или выявил новое требование.
- **`ROADMAP.md`** — переключи чекбокс если change закрыл запланированный пункт.
